import {
  Body, Controller, Get, HttpException, HttpStatus, Inject, Param, Post,
  Put, Req
} from '@nestjs/common';
...
export class UsersController {
  ...
  public async getUserByToken(
    ...
    const userResponse: IServiceUserGetByIdResponse = await firstValueFrom(
      this.userServiceClient.send({cmd: 'user_get_by_id'}, userInfo.id),
    );
  ...
  public async createUser(
    ...
    const createTokenResponse: IServiveTokenCreateResponse = await firstValueFrom(
      this.tokenServiceClient.send({cmd: 'token_create'}, {
  ...
  public async loginUser(
    ...
    const getUserResponse: IServiceUserSearchResponse = await firstValueFrom(
      this.userServiceClient.send({cmd: 'user_search_by_credentials'}, loginRequest),
    );
  ...
  public async logoutUser(
    ...
    const destroyTokenResponse: IServiceTokenDestroyResponse = await firstValueFrom(
      this.tokenServiceClient.send({cmd: 'token_destroy'}, {
        userId: userInfo.id,
      }),
    );
  ...
  public async confirmUser(
    ...
    const confirmUserResponse: IServiceUserConfirmResponse = await firstValueFrom(
      this.userServiceClient.send({cmd: 'user_confirm'}, {
        link: params.link,
      }),
    );
